    ...
    if (elts.count >= 2) {
        ...
        for (NSString *param in urls) {
            if ([param isEqualToString:@"_wx_tpl"]) {
                transformURL =[[urls lastObject] stringByRemovingPercentEncoding];//  [[urls lastObject]  stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
                break;
            }
        }
    }
...
- (BOOL)remoteDebug:(NSURL *)url
{
    ...
    for (NSString *param in [query componentsSeparatedByString:@"&"]) {
        ...
        if ([[elts firstObject] isEqualToString:@"_wx_debug"]) {
            ...
            [WXSDKEngine connectDebugServer:[[elts lastObject]  stringByRemovingPercentEncoding]]; //stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding
            ...
            NSString *devToolURL = [[elts lastObject]  stringByRemovingPercentEncoding];//stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding
